...
def normalize_analysis_data(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    enum_mappings = {
        ...
        'style': {
            ...
            'professional': Style.MODERN,  # Map professional style to modern
        },
        'visual_theme': {
    ...
    enum_defaults = {
        ...
        'imagery_style': ImageryStyle.REALISTIC
    }
    ...
    for field, mapping in enum_mappings.items():
        if field in normalized:
            ...
            default_val = enum_defaults.get(field, Tone.FRIENDLY)
            
            if isinstance(value, str):
                # Try to find the value in the mapping
                # If not found, use the default for this specific field
                normalized[field] = mapping.get(value.lower(), default_val)
            else:
                # If not a string, use the default for this specific field
